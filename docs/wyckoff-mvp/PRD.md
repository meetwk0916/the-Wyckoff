...
- 先在 ptrade 内打通回测、模拟盘和交易报告闭环，再逐步把 L2 订单流沉淀为可消费的数据契约。
...
- 启动 ptrade Phase 1：原生回测、模拟盘与交易报告
- 启动 ptrade Phase 2：L2 订单流接入、标准化与录制 / 回放
...
1. Phase 0：先做账号、L2 权限、网络边界和本地落盘的环境预检查。
2. Phase 1：优先完成 ptrade 原生回测、模拟盘和交易报告闭环。
3. Phase 2：在闭环稳定后，再把 L2 订单流接回同一策略链路做信号增强。
