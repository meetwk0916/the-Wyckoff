...
- 优先获取 Wyckoff 2.0 所需的 L2 订单流核心数据，并沉淀为可消费的数据契约。
...
- 启动 ptrade Phase 1：L2 订单流接入、标准化与录制 / 回放
...
1. Phase 1：优先获取 L2 订单流，这是当前判断下 Wyckoff 2.0 最关键的新增数据能力。
2. Phase 2：在 L2 数据稳定后，再扩展其他 API 能力，例如回测、研究接口与历史回放。
3. Phase 3：最后才进入自动交易、执行反馈与风控闭环。
