...
`crypto:phase-c:candidates` 支持 `--since`、`--until` 和 `--lookback-hours` 限定候选锚点时间；扫描仍会自动读取 `before-min` / `after-min` 上下文窗口，适合 raw JSONL 继续增长后的日常巡检。
...
`crypto:phase-c:watch` 会运行 daily-check、unreviewed 和 review-next，输出 source health、候选数量、最佳 long-liquidation cluster、review-next 状态和单条下一步建议，并透传候选扫描时间窗口参数。
`crypto:phase-c:watch:export` 会把 watch 结果同步到 `public/mock/crypto-phase-c-watch.json`，供前端 BTC Phase C 监控页读取，并透传候选扫描时间窗口参数。
