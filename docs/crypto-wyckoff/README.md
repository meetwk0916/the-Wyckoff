...
当前结论：crypto 路线应作为独立工作面推进，后续可以新增 `crypto-workspace/`，但不要混进 `ptrade-workspace/`。ptrade 仍然服务 A 股券商运行时；crypto 路线服务 BTC 24/7 市场、永续合约、跨交易所订单流和衍生品指标。
...
- 后续如新增 `crypto-workspace/`，它应承担采集、落盘、回放、paper trade 和交易所 sandbox 验证。
