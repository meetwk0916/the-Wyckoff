...
当前结论：crypto 路线应作为独立工作面推进，当前已经有独立 `crypto-workspace/`，不要混进 `ptrade-workspace/`。ptrade 仍然服务 A 股券商运行时；crypto 路线服务 BTC 24/7 市场、永续合约、跨交易所订单流和衍生品指标。
...
- `crypto-workspace/` 当前承担 Phase 0 供应商验证、落盘边界和探测脚本；后续再继续扩展到回放、paper trade 和交易所 sandbox 验证。
