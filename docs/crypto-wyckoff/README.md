...
截至 2026-05-13，Phase C evidence 已加入启发式结构上下文：从窗口内 spot / perp trade 和 book mid 观测估算局部支撑 / 阻力，并记录跌破后是否收回。该字段只用于候选解释和人工复核，不输出交易动作。
...
已新增 Phase C 候选窗口扫描：从本地 raw JSONL 自动寻找 BTC liquidation 窗口并生成 fixture draft。当前本地数据只有 1 个 BTC liquidation，且为 short liquidation 对照样本；仍缺少 long liquidation 正样本，因此还没有 `spring_candidate` 样本是正常状态。

已把 Bybit `allLiquidation.BTCUSDT` 接入为免费实时 liquidation-only 补充源。当前推荐使用心跳版长跑 session `wyckoff_bybit_liq_capture_24h_heartbeat`，并用 `npm run crypto:capture:status -- --screen=wyckoff_bybit_liq_capture_24h_heartbeat` 监控；trade、book、OI 和 Funding 上下文仍由 Binance / OKX 或历史导入补齐。
