...
截至 2026-05-11，Phase C evidence 已加入启发式结构上下文：从窗口内 spot / perp trade 和 book mid 观测估算局部支撑 / 阻力，并记录跌破后是否收回。该字段只用于候选解释和人工复核，不输出交易动作。

同日已新增 Phase C review index 与规则评分报告，用来把人工复核沉淀成机器可读标签、理由和因子。它服务规则校准，不是上线后的逐笔人工审批。

同日已新增第一版窗口级 spot / perp CVD 判据：每个窗口会输出 CVD notional delta、delta ratio、demand / supply bias、spot-perp divergence 和 Phase C flow support。该判据进入候选过滤和规则评分，但阈值仍需要通过更多复核样本校准。

同日已新增 Phase C 候选窗口扫描：从本地 raw JSONL 自动寻找 BTC liquidation 窗口并生成 fixture draft。当前本地数据只有 1 个 BTC liquidation，且为 short liquidation 对照样本；仍缺少 long liquidation 正样本。

同日已把 Bybit `allLiquidation.BTCUSDT` 接入为免费实时 liquidation-only 补充源。它用于提高长跑采集命中 BTC long liquidation 样本的概率；trade、book、OI 和 Funding 上下文仍由 Binance / OKX 或历史导入补齐。
