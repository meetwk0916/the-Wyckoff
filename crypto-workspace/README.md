...
`crypto:daily-check` also prints a source health table for OKX trade, OKX book, OKX liquidation, Binance forceOrder, Bybit liquidation, Binance OI / Funding, and OKX OI / Funding. Stale, errored, missing-status, or not-running sources are also promoted into `attention.sourceIssues`, so a stale liquidation-only stream is not hidden by fresh trade / book or REST derivatives payloads from another screen.
...
npm run crypto:phase-c:candidates -- --lookback-hours=24
npm run crypto:phase-c:candidates -- --since=2026-05-17T14:00:00Z --until=2026-05-17T14:20:00Z
npm run crypto:phase-c:candidates -- --no-cluster
npm run crypto:phase-c:unreviewed
npm run crypto:phase-c:review-next
npm run crypto:phase-c:watch -- --lookback-hours=24
npm run crypto:phase-c:watch:export -- --lookback-hours=24
```

The candidate scan report is written to `crypto-workspace/reports/phase-c-candidates-last.json`. It finds BTC liquidation events, defaults to merging overlapping same-provider / same-direction events into clusters, builds review windows around those clusters, checks trade / book / OI / Funding / liquidation coverage, and emits fixture drafts. Use `--since`, `--until`, or `--lookback-hours` to limit candidate anchors for daily monitoring as raw JSONL grows; the scanner still reads the configured `before-min` / `after-min` context around matching anchors. Use `--no-cluster` only when debugging raw event-level duplication. It does not classify Spring or approve trades.
...
`crypto:phase-c:watch` is the compact daily operator view. It runs daily-check, unreviewed, and review-next, then writes `reports/phase-c-watch-last.json` with source health, candidate counts, the best long-liquidation cluster, review-next status, and a single next action. It accepts the same `--since`, `--until`, and `--lookback-hours` candidate-scan filters through `daily-check`.
`crypto:phase-c:watch:export` reruns watch and writes `public/mock/crypto-phase-c-watch.json`, which powers the frontend BTC Phase C monitoring page. It also accepts the same candidate-scan time filters.
